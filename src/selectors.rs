...
//! BackupCatalog 中的条目；介质归属由 application 层 canonical matcher 决定。
...
impl BackupSelector {
    ...
    pub fn numbered(&self) -> Vec<&BackupEntry> {
        numbered_entries(self.catalog.entries())
    }
...
fn resolve_one<'a>(catalog: &'a BackupCatalog, target: &str) -> Result<&'a BackupEntry, String> {
    if !target.contains(',') && !target.contains('-') && target.bytes().all(|b| b.is_ascii_digit())
    {
        let entries = numbered_entries(catalog.entries());
        let index = target
            .parse::<usize>()
...
fn resolve_many<'a>(
    ...
    let numbered = numbered_entries(catalog.entries());
    ...
    for target in targets {
        if numeric_selection(target) {
            ...
            let entry = resolve_one(catalog, target)?;
            let canonical = backup_catalog::canonical_entry_path(&entry.path);
            if paths.insert(canonical) {
                out.push(entry);
            }
