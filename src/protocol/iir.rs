//! Independent SectorManageImp IIR plaintext helpers.
//!
//! IIR is a separate protocol object from the LBA7 legacy compatibility extent.
...
impl std::error::Error for IirError {}
...
pub fn analyze_iir_plain(data: &[u8]) -> Result<IirPlainAnalysis, IirError> {
    if data.len() != IIR_MAIN_SIZE {
        return Err(IirError::WrongIirPlainLength {
