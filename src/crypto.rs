...
pub const SBOX2: [u8; 256] = [
    ...
    156, 239, 160, 224, 59, 77, 174, 42, 245, 176, 200, 235, 187, 60, 131, 83, 153, 97, 23, 43, 4,
...
fn aes_expand(key16: &[u8; 16]) -> [[u8; 4]; 44] {
    let mut w = [[0u8; 4]; 44];
    for i in 0..4 {
        w[i] = [key16[i * 4], key16[i * 4 + 1], key16[i * 4 + 2], key16[i * 4 + 3]];
    }
    for i in 4..44 {
        let mut t = w[i - 1];
        if i % 4 == 0 {
...
mod tests {
    ...
    fn lba6_decode_splits_checksum() {
        let raw: Vec<u8> = (0..256).map(|i| i as u8).chain((0..256).map(|i| i as u8)).collect();
