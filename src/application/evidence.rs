...
impl EvidenceSource {
    pub fn open_backup(path: &Path) -> Result<Self, String> {
        ...
        if protocol.len() != METADATA_IMAGE_LEN {
            return Err(format!(
                "EDPB LBA0-12 大小 {}B，预期 {METADATA_IMAGE_LEN}B",
                protocol.len()
            ));
        }
    ...
    pub fn open_disk(runner: &dyn CmdRunner, disk: u32) -> Result<Self, String> {
        let target =
            TargetSession::<ReadOnly>::open_usb(runner, disk).map_err(|error| error.msg)?;
        let total_sectors = target
            .total_sectors()
            .ok_or_else(|| "无法取得设备总扇区数".to_string())?;
        let path = diskio::raw_path(disk);
        let mut dev = FileDev::open_rdonly(&path)
            .map_err(|error| format!("无法只读打开 disk{disk}: {error}"))?;
        let protocol = dev
