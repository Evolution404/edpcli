...
def main() -> None:
    ...
    region_offset = chs_sectors * sector_size - REGION_BACKOFF_BYTES
    physical_tail_offset = total_bytes - REGION_BACKOFF_BYTES
    device = f"/dev/r{args.disk}"
    fd = os.open(device, os.O_RDONLY | os.O_CLOEXEC)
    try:
        lba7 = os.pread(fd, sector_size, 7 * sector_size)
        region_a = os.pread(fd, REGION_SIZE, region_offset)
        neighborhood = os.pread(fd, REGION_SIZE + 0x1000, region_offset - 0x800)
        physical_tail_candidate = os.pread(fd, REGION_SIZE, physical_tail_offset)
    finally:
        os.close(fd)
    if any(len(data) != length for data, length in (
        (lba7, sector_size), (region_a, REGION_SIZE),
        (neighborhood, REGION_SIZE + 0x1000),
        (physical_tail_candidate, REGION_SIZE),
    )):
    ...
    files = {
        "lba7_raw.bin": lba7,
        "region_a_chs.bin": region_a,
        "region_a_neighborhood.bin": neighborhood,
    ...
    manifest = {
        ...
        "region_a_lba": region_offset // sector_size,
        "region_a_byte_offset": region_offset,
