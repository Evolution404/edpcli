...
- 本轮定向门禁：`application::inspect` 6/6、`inspect_cli` 6/6、`tui_inspect_workspace` 5/5、`tui_lifecycle` 13/13、`tui_state` 37/37 通过，`cargo check --all-targets` 与 `git diff --check` 通过。下一步进入 Phase I3：建立全盘 `InspectNode / Region / Extent / Sector / Field` lazy 模型，并把 LBA0～12、LCE、partitions、data、tail、unknown range 全部纳入统一树。
