...
- `AdvancedInspectWorkspace` 已直接携带同源 topology，因此 CLI/TUI 后续不需要重新推导磁盘区域。I3 定向门禁：tree 5/5、application inspect 11/11、CLI 6/6、TUI workspace 5/5、TUI lifecycle 13/13，`cargo check --all-targets` 与 `git diff --check` 通过。下一步进入 Phase I4：用 topology 替换当前 Advanced Inspect 表单/结果双轨，落地树状 TUI、`o` 展开折叠、j/k、Enter、Tab/Shift+Tab 与 viewport virtualization。
