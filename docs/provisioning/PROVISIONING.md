...
- I6 专项门禁：Sector Inspector 4/4 通过，覆盖 Field→Hex、Hex→Field、跨行/跨 sector range、Unknown/Reserved/Preserved、yank register；完整 `cargo test --all-targets` 通过（最终长时 Runner Job `exit_code=0`），`cargo fmt --all -- --check`、`cargo check --all-targets` 与 `git diff --check` 通过。下一步进入 Phase I7：实现 `g` LBA/offset Jump、`/` 结构化字段搜索和自动树定位，raw full-disk pattern search 仍留到第二阶段。
