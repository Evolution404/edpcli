...
|---|---|
| `j / k` | 上 / 下选择 |
| `Tab / h / l` | 在设备、备份两个标签页循环切换；检查界面中切换视图 |
| `Enter` | 在设备页进入所选 USB 盘的制盘流程 |
| `gg / G` | 首项 / 末项 |
...
制盘流程包含五个入口：官方模式 0/1/2/3 和离线 LBA 快照转换；不再提供单独的“改造”模式。
从设备页按 Enter 进入制盘流程时，会先显示当前目标盘此前是否已有 EDPB 保存记录，并让用户明确选择“先保存当前盘”或“不保存直接继续”。保存完成或用户明确跳过后才进入模式选择。
四种物理模式先填写分区/身份/密码参数，再生成只读计划。TUI 默认密码为 `0000aaaa`，
...
范围以及当前布局的剩余空间或超出空间。按 `Space` 可切换为“按比例分配”，此时输入当前模式
...
硬件身份绑定的稀疏镜像，真实写盘仍需再次输入 `YES`。模式1若识别到目标盘已经是模式0，会自动改用保留重制计划：原 type4 的起点、大小和密钥材料保持不变，type2 从 LBA63 扩满到原 type4 之前；不会再出现单独的改造入口。离线快照转换不要求插盘，
...
设备/备份扫描、快速/高级检查、制盘计划、稀疏镜像导出和离线转换都在后台执行；
