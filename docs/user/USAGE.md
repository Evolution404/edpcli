...
| 键位 | 行为 |
|---|---|
| `j / k / h / l` | 当前列表、树、十六进制视图或局部控件的 类 Vim 导航，不切工作区 |
| `gg / G` | 当前列表或树的首项 / 末项 |
| `Ctrl-d / Ctrl-u` | 当前列表、树或详情半页移动 |
| `Tab / Shift-Tab` | 在顶层“设备 / 备份”两个标签间切换 |
| `Ctrl-w h/j/k/l/w/W` | 当前功能页内部的面板方向或循环切换 |
| `/` | 搜索当前设备、备份或检查内容 |
| `n / N` | 下一个 / 上一个搜索匹配 |
| `:` | 打开任务型命令面板，不执行 shell |
| `r` | 刷新当前工作区 |
| `a` | 设备页或备份页新建备份，并选择元数据备份 / 深度备份 |
| `p` | 设备页进入制盘；制盘表单中生成/预览计划 |
...
| `Enter` | 当前页默认动作；制盘 Form 的 Normal 模式生成只读计划，Insert 模式完成当前字段编辑 |
| `Esc` | 返回上一层 / 退出当前输入模式 |
| `q` | 退出 |
| `?` | 帮助 |

制盘流程当前统一包含五种目标状态：官方 mode0/mode1/mode2/mode3，以及“恢复普通盘（Plain）”；不再提供单独的“改造”或 Offline Convert 模式。
从设备页按 `p` 或 Enter 进入制盘流程时，会先显示当前目标盘此前是否已有 EDPB 保存记录，并让用户明确选择“先保存当前盘”或“不保存直接继续”。保存完成或用户明确跳过后才进入模式选择。
...
TUI 只有一个用户可见的检查入口：`:inspect`、设备/备份页 `i` 以及备份页 Enter 都进入同一个**全盘结构树**。树可展开“设备 → 区域 → 范围 → 扇区 → 字段”，`o`/`h`/`l` 折叠展开，`gl` 跳转任意 LBA/绝对字节偏移，`/` 搜索结构化节点或已缓存字段/值，`n/N` 在全部匹配间循环；扇区检查器支持 `0/$` 行首尾、`gg/G` 扇区首尾、`Ctrl-u/Ctrl-d` 半页、PageUp/PageDown 跨扇区、`v` 切换原始/解码/混合，以及 `Space/o` 展开字段/位。
