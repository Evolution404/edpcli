...
| `gt / gT` | 下一个 / 上一个工作区 |
| `gd / gb / gp / gi` | 直达设备 / 备份 / 制盘 / 检查 |
| `Tab / Shift-Tab` | 下一个 / 上一个 面板，不切工作区 |
| `Ctrl-w h/j/k/l/w/W` | 面板 方向或循环切换 |
| `/` | 搜索当前设备、备份或检查内容 |
| `n / N` | 下一个 / 上一个搜索匹配 |
| `:` | 打开任务型命令面板，不执行 shell |
| `r` | 刷新当前工作区 |
| `a` | 新建备份并选择 元数据备份 / 深度备份 |
| `空格` | 勾选 / 取消当前备份，用于固定路径+SHA-256 的多目标删除 |
| `d` | 删除当前或已勾选备份，统一进入安全确认 |
| `v` | 备份页校验；扇区检查器中循环 原始 / 解码 / 混合 |
| `gl` | 检查跳转；单按 `g` 不执行跳转 |
| `i` | 制盘 Normal 模式下编辑当前可编辑字段，进入 Insert；Insert 内普通字母只作为文本 |
| `Enter` | 制盘 Form 的 Normal 模式生成只读计划；Insert 模式完成当前字段编辑 |
| `Esc` | 返回上一层 / 退出当前输入模式 |
| `q` | 退出 |
| `?` | 帮助 |
...
TUI 只有一个用户可见的检查入口：`:inspect`、`gi` 以及备份页 Enter 都进入同一个**全盘结构树**。树可展开“设备 → 区域 → 范围 → 扇区 → 字段”，`o`/`h`/`l` 折叠展开，`gl` 跳转任意 LBA/绝对字节偏移，`/` 搜索结构化节点或已缓存字段/值，`n/N` 在全部匹配间循环；扇区检查器支持 `0/$` 行首尾、`gg/G` 扇区首尾、`Ctrl-u/Ctrl-d` 半页、PageUp/PageDown 跨扇区、`v` 切换原始/解码/混合，以及 `Space/o` 展开字段/位。
