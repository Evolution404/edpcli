...
- **COMPLETE：4731B / 6656B = 71.1%**
- **PARTIAL：1925B / 6656B = 28.9%**
...
| LBA4 | 0x01C–0x01F | PARTIAL | `OnllyID2Nd` = backup/activation encryption key seed | **current producer**：Windows `RegsiterUsb@0x1003BBD1..0x1003BBD7` 直接执行 `node+0x04 = object+0x698`，后者就是本次注册 main onlyid；因此 current profile 第二 key 与 main onlyid 相同。**legacy producer**仍未定位，真实 legacy 样本使用独立4B值 | active consumer 已闭合为完整 round-trip：`GetUpLoadInformation/sub_10039C30` 组装 LLGB+EDPF backup blob 后，以 `restore_node+0x04` 作为4B key seed 调 `sub_10001190` 加密；`ActiveNormalUDev -> sub_1003CEB0` 取同一 `node+0x04` 调 `sub_100012D0` 解密服务器/恢复 blob，要求 plaintext 以 `LLGB` 开头，再把 LLGB 标签恢复到 LBA8，并按 v0x0202/v0x0206 分区表恢复 LBA12。两 helper 的16B key material 都是 `key4[i mod 4] XOR "EDPSECDISK200709"[i]`，随后走同一 key schedule、互为 encrypt/decrypt block transform | strict current profile `OnllyID2Nd==main onlyid && HSerialCRC=0`；legacy profile `OnllyID2Nd!=main`。历史22份中15个可直接按文件名复核的 legacy restore node里，second key 没有一个等于本设备或全集任何 main-onlyid；同一 main 的重复备份 second key 稳定。committed NETAC_A/NETAC_B/LEXAR 分别锁定 second=`44D9CE02/028EFFD3/7647B1EF` | 字段的加密用途、双向 consumer、current producer 和真实 legacy wire 值均已闭合；严格 blocker 只剩 **legacy second-key 的生成/注入 producer**，因此4B继续 PARTIAL |
...
| LBA4 | 0x035–0x038 | PARTIAL | `MyHardinfo` = observed mirror of LBA8 `HDSerialInfo` | current SAFE6 `RegsiterUsb` 在 `0x1003BBBB` 对完整0x2F restore node `memset(0)`，后续显式写相邻字段但从不覆盖 `node+0x1D..20`，因此 current producer 明确为0；legacy 非零 producer尚未定位 | Windows/Linux `ReadSector4` 结构保留整个node但不按该DWORD分支；LBA8 runtime则结构保存同值 `HDSerialInfo`。当前没有证明 LBA4 副本自身参与行为判断 | strict originals 逐盘比较得到 **22/22 `MyHardinfo == LBA8.HDSerialInfo`**，同时保留 current `0->0` 和 legacy 非零 `A017AD78/A68BAE08/8B4613F5/2AB0E33C` 多profile；该值不等于 device-id CRC 或 MBR signature | 字段角色已从 opaque DWORD 收敛为 HDSerialInfo 镜像/兼容副本；current-zero producer与跨扇区镜像关系闭合，但 legacy 非零值的直接 copy/producer 及 profile selection 仍缺，因此4B继续PARTIAL |
...
| LBA6 | 0x1C9–0x1CF | PARTIAL | GSerial post-NUL / long-string tail 与 legacy MBR underlay overlap | current writer仍复制输入槽前15B且第16B显式为0；short profile在 `+0x1C8` NUL后进入上游 backing，legacy 两盘又保存旧 MBR entry1/2 underlay；long profile则继续是 GSerial 字符串 | reader只解释首个NUL前字符串，不消费 short profile 的尾 backing | 22盘真实 short/long 分叉及两份 legacy MBR underlay | 7B存在真正的 profile-dependent 物理语义；旧三分区 MBR underlay writer仍缺，继续PARTIAL |
...
| LBA6 | 0x1D1–0x1DF | PARTIAL | BeiZhu remainder + profile-dependent post-NUL backing / legacy MBR underlay | current writer固定复制输入前15B；空串 profile 从 byte1 起已经可能是 backing，legacy“普通”profile在NUL后又与旧 MBR entry2/entry3 underlay连续 | reader只消费到首个NUL | 22盘：20空、2份GBK“普通”；8/22首个NUL后有非零backing | 15B继续PARTIAL，等待旧三分区 MBR underlay producer闭合 |
