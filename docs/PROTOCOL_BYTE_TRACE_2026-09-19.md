...
- **COMPLETE：5530B / 6656B = 83.1%**
- **PARTIAL：1126B / 6656B = 16.9%**
...
| LBA6 | 0x1C9–0x1CE | PARTIAL | GSerial post-NUL / long-string tail 与 legacy MBR underlay overlap | current writer固定复制输入槽前15B；short profile在 `+0x1C8` NUL后进入上游 backing，legacy 两盘又保存旧 MBR entry1/2 underlay；long profile则继续是 GSerial 字符串 | reader只解释首个NUL前字符串，不消费 short profile 的尾 backing | 22盘真实 short/long 分叉及两份 legacy MBR underlay | 6B存在真正的 profile-dependent 物理语义；旧三分区 MBR underlay writer仍缺，继续PARTIAL |
