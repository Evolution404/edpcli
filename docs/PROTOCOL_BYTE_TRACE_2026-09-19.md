...
| LBA6 | 0x188–0x1BF | PARTIAL | m_usbLabel fixed 56B physical slot | Windows `sub_10013FD0` / Linux `BuildSector6` 固定从 `UsbWriteParam.m_usbLabel+0x218` 复制0x38B到 `out+0x188` | Linux `ReadSector6@0x1E84B..` 从 `decoded+0x188` 构造 C++ string，再写回 `UsbLabelParam.m_usbLabel[64]`；Windows reader同构 | committed原始夹具的 LBA6 C-string 与 LBA8 `Label=` 一致；22盘均可见 `江苏电力!SAFE6`，但 NUL 后 backing 存在多profile | 字符串与56B物理边界闭合，post-NUL backing仍非语义字段，PARTIAL |
