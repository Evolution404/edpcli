...
3. LBA4 `HSerialCRC[5]` is COMPLETE as a caller-owned five-DWORD identity vector, corresponding to `UsbLabelParam::HDOnlySerial[5]`. The field boundary, current absent-zero profile, v19 request injection ABI, official writer transport, three restore-node mirror readers, value-ignore restore consumer, and real nonzero profiles are closed; the v19 memory-backed writer also reproduces the authentic SanDisk LBA4 bit-exact while preserving its nonzero 20B vector. `DeviceNumber.dll` still returns only one DWORD and is explicitly **not** the HSerial generation algorithm. The earlier caller's value-generation algorithm remains provenance research, not a wire-field semantic gap.
